...
import rclpy
from bb_controls_msgs.msg import ThrusterForces
...
class ThrustRepublisher(Node):
    def __init__(self):
        ...
        self.thruster_sub = self.create_subscription(
            ThrusterForces, "/auv4/thrusters/forces", self.thruster_callback, 10
        )
    ...
    def thruster_callback(self, msg: ThrusterForces):
        ...
        if len(msg.thrusts) != 7:
            self.get_logger().warn(
                f"ThrustRepublisher: Expected 7 values, but got {len(msg.thrusts)}"
            )
            return

        thruster_mappings = [0, 1, 2, 3, 4, 5, 6]

        for i in range(7):
            force_msg = Float64()
            force_msg.data = float(msg.thrusts[thruster_mappings[i]])
